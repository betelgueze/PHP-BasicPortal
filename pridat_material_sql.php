...
if(!empty($_POST["mnozstvi"]) and !empty($_POST["cena"]) and !empty($_POST["typ"]) and !empty($_POST["splatnost_do"])) 
{ // pokud jsou vsechny udaje vyplnene
	if(!is_numeric($_POST["mnozstvi"]) or !is_numeric($_POST["cena"]) or is_numeric($_POST["typ"]))
	...
	{ // vsechno je zadano spravne
		...
		{ // pokud prave pracuje na nejake zakazce
			$vysledek = "INSERT INTO material (Cena, Typ, Mnozstvi, fk_zakazka) VALUES ('".$_POST["cena"]."', '".$_POST["typ"]."', '".$_POST["mnozstvi"]."', '".$_SESSION["id_zak2"]."')";
			mysql_query($vysledek);
			$vysledek2 = "INSERT INTO faktura (Hodnota_faktury, Datum_splatnosti, fk_zakazka) VALUES ('".$_POST["cena"]."', '".$_POST["splatnost_do"]."', '".$_SESSION["id_zak2"]."')";
			mysql_query($vysledek2); 
		}
		header("HTTP/1.1 301 Moved Permanently");
		header("Location: material_zobraz.php");
