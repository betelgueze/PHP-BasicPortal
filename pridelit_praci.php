...
<TR height="70%">
	...
	<TD width="85%" colspan="2" align="center" valign="center"><h2>Pridelit praci:</h2><br>
		<?php // formular s loginama
			...
			$vysledek = mysql_query("SELECT * FROM hesla WHERE JeAdmin = 3"); // jen zamestnanci
		?>
		Vyberte zamestnance kteremu chcete pridelit praci: <form method="post" action="pridel2.php">
	<select name="zamestnanec">
<?php
  while($zamestnanec = mysql_fetch_assoc($vysledek)){?>
 <option value="<?php echo $zamestnanec["Login"];?>"><?php echo $zamestnanec["Login"];?></option> 
<?php
  }  
  mysql_free_result($vysledek);                                    
?>
  <input type="submit"  value="Vybrat"> </form><BR>
