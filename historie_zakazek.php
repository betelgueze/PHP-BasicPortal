...
<TR height="70%">
	...
	<TD width="85%" colspan="2" align="center" valign="center"><h2>Historie mych zakazek:</h2><br>
		...
		<?php
			...
			while ($zaznam = MySQL_Fetch_Array($vysledek)) // vsechny zakazky v historii, na kterych zamestnanec delal
			{
				...
				{
					$vysledek3 = mysql_query("SELECT * FROM zakaznik WHERE RC = '".$zaznam2["fk_zakaznik"]."'");
					$zaznam3 = MySQL_Fetch_Array($vysledek3);
					if($zaznam["fk_zakazka"] == $zaznam2["ID"])
					{ // pokud je zamestnancova nejaka zakazka neaktivni
						?> 
							<tr height="5%">
							<td width="5%"><?php echo $zaznam2["ID"]; ?> </td>
							<td width="5%"><?php echo $zaznam3["Jmeno"]." ".$zaznam3["Prijmeni"].""; ?> </td>
							<td width="30%"><?php echo $zaznam2["Popis"]; ?> </td>	
						<?php
					}
				}
			}
